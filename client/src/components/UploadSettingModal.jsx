...
const UploadSettingModal = ({ hideModal }) => {
    ...
    if (e.target.files[0]) {
      ...
      try {
        const response = await fetch(
          "https://48d7-88-99-162-157.ngrok-free.app/generateDoc",
          {
            method: "POST",
            body: formData,
          }
  ...
  return (
    ...
    >
      <div className="w-2/3 md:w-1/2 lg:w-1/3 h-2/3 px-12 py-16 flex flex-col items-center bg-white shadow-md shadow-gray-500 rounded-md relative">
        <IoClose
        ...
        <div className="w-full flex flex-col gap-6">
          ...
          <Button
            ...
            className="border border-gray-200 py-2 text-center focus:outline-none cursor-pointer w-full select-none"
            role="button"
            tabIndex="0"
            severity="info"
          >
            <label htmlFor="upload" className="w-full">
              <span className="w-full">Select File & Upload</span>
              <input
                id="upload"
                type="file"
                className="hidden w-full"
                onChange={handleFileChange}
                disabled={!enableUpload} // Ensure the input is only enabled when appropriate
              />
            </label>
          </Button>
        </div>
